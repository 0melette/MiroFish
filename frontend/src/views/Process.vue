            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, the LLM analyses the content and automatically generates an ontology structure (entity types + relation types) suitable for opinion simulation
                </div>
              </div>
...
const pollTaskStatus = async (taskId) => {
  try {
    ...
    if (response.success) {
      ...
      // Task not found (404) — fall back to checking project status.
      // The in-memory TaskManager may have been cleared by a server restart.
      console.warn('Task not found, checking project status as fallback...')
      const projectResponse = await getProject(currentProjectId.value)
      if (projectResponse.success) {
        projectData.value = projectResponse.data
        // Treat as completed if explicitly graph_completed, or if graph_building
        // but graph_id already exists (backend restarted before final status save).
        const isDone = projectResponse.data.status === 'graph_completed' ||
          (projectResponse.data.status === 'graph_building' && projectResponse.data.graph_id)
        if (isDone && projectResponse.data.graph_id) {
          console.log('✅ Build completed (detected via project status)')
          stopPolling()
          stopGraphPolling()
          currentPhase.value = 2
          buildProgress.value = { progress: 100, message: '构建完成，正在加载图谱...' }
          await loadGraph(projectResponse.data.graph_id)
          buildProgress.value = null
        } else if (projectResponse.data.status === 'failed') {
          stopPolling()
          stopGraphPolling()
          error.value = 'Graph construction failed: ' + (projectResponse.data.error || 'Unknown error')
          buildProgress.value = null
        }

...
class GraphBuilderService:
    """
    Graph builder service.
    Calls the Zep API to construct a knowledge graph.
    """
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # This usage is required by the Zep SDK; the warnings from dynamic class creation are safe to ignore.
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            # Create attribute dict and type annotations (Pydantic v2 required).
            ...
            for attr_def in entity_def.get("attributes", []):
                ...
                # The Zep API needs a Field with a description; this is required.
    ...
    def add_text_batches(
        ...
        """Add text to the graph in batches and return a list of episode UUIDs."""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        """Wait for all episodes to be processed (by polling each episode's processed status)."""
        ...
        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"Timed out — completed {completed_count}/{total_episodes} chunks",
            ...
            for ep_uuid in list(pending_episodes):
                try:
                    ...
                    # Ignore individual query errors; keep going.
                    pass
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        Returns:
            A dict containing nodes and edges with time info, attributes, etc.
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # Build a node map for resolving node names.
        node_map = {}
        for node in nodes:

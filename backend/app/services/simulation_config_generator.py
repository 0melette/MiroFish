...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        logger.info(f"Simulation config generation complete: {len(params.agent_configs)} agent config(s)")
    ...
    def _assign_initial_post_agents(
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                logger.warning(f"No agent matching type '{poster_type}' found, using highest-influence agent")
                if agent_configs:
                    # 按影响力排序，选择影响力最高的

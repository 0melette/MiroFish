...
Step 2: Zep entity reading and filtering, plus OASIS simulation preparation and execution.
"""
...
# Adding this prefix discourages tool calls and asks the Agent to reply directly in text.
INTERVIEW_PROMPT_PREFIX = "Based on your persona, past memories, and prior actions, reply directly in plain text without calling any tools: "


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize the interview prompt by adding a prefix that discourages tool usage.
...
# ============== Entity reading routes ==============

@simulation_bp.route('/entities/<graph_id>', methods=['GET'])
def get_graph_entities(graph_id: str):
    """
    Fetch all filtered entities from the graph.
    
    Only nodes that match predefined entity types are returned.
    These are nodes whose labels include more than just Entity.
    
    Query params:
        entity_types: Comma-separated list of entity types for additional filtering.
        enrich: Whether to fetch related edge data. Defaults to true.
    """
    try:
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    """Fetch details for a single entity."""
    try:
        if not Config.ZEP_API_KEY:
            return jsonify({
                "success": False,
...
def create_simulation():
    ...
    Note: max_rounds and related parameters are generated by the LLM, so they do not need to be set manually.
    
    Request JSON:
        {
            "project_id": "proj_xxxx",      // required
            "graph_id": "mirofish_xxxx",    // optional, falls back to the project graph
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Conditions:
    1. state.json exists and has status set to "ready".
    2. Required files exist: reddit_profiles.json, twitter_profiles.csv, simulation_config.json.
    
    Note: run_*.py scripts remain in backend/scripts/ and are no longer copied into the simulation directory.
    ...
    # Required files, excluding scripts stored under backend/scripts/.
    required_files = [
        "state.json",
    ...
    # Inspect the state stored in state.json.
    ...
    try:
        ...
        # Treat the simulation as prepared if config_generated is true and the files exist.
        # All of the following statuses imply preparation has completed:
        # - ready: preparation completed and ready to run
        # - preparing: config_generated=true means preparation is effectively complete
        # - running: simulation is already running, so preparation finished earlier
        # - completed: simulation finished, so preparation finished earlier
        # - stopped: simulation was stopped, so preparation finished earlier
        # - failed: runtime failed, but preparation did complete
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If state is still preparing but all files are done, promote it to ready automatically.
            if status == "preparing":
                try:
            ...
            logger.warning(f"Simulation {simulation_id} prepared-state result: not ready (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    ...
    请求（JSON）：
        {
            ...
            "max_agents": 5,                               // 可选，限制最终Agent总数
            "selected_entity_uuids": ["uuid1", "uuid2"], // 可选，精确指定实体
            "profile_overrides": {                         // 可选，覆盖生成后的人设字段
                "uuid1": {
    ...
    try:
        ...
        selected_entity_uuids = data.get('selected_entity_uuids') or []
        if not isinstance(selected_entity_uuids, list):
            return jsonify({
                "success": False,
                "error": "selected_entity_uuids must be an array"
            }), 400
        selected_entity_uuids = [str(entity_uuid).strip() for entity_uuid in selected_entity_uuids if str(entity_uuid).strip()]

        profile_overrides = data.get('profile_overrides') or {}
        if not isinstance(profile_overrides, dict):
            return jsonify({
                "success": False,
                "error": "profile_overrides must be an object"
            }), 400

        logger.info(
            "Starting /prepare request: simulation_id=%s, force_regenerate=%s, max_agents=%s, selected_entities=%s, overrides=%s",
            simulation_id,
            force_regenerate,
            max_agents,
            len(selected_entity_uuids),
            len(profile_overrides)
        )
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Preparation already complete, skipping re-generation",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            else:
                logger.info(f"Simulation {simulation_id} not yet prepared, launching prepare task")
        ...
        try:
            ...
            resolved_preview_entities = manager._resolve_prepare_entities(
            ...
            # 保存实体数量到状态（供前端立即获取）
            state.entities_count = len(resolved_preview_entities)
            state.entity_types = list({
                entity.get_entity_type() or "Entity"
                for entity in resolved_preview_entities
            })
            logger.info(
                "Expected entity count: %s, preview filtered entities: %s, types: %s",
        ...
        # Update simulation state, including the precomputed entity count.
            ...
            try:
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    stage_names = {
                        ...
                        "generating_profiles": "Generate Agent profiles",
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "Preparation task started. Check progress through /api/simulation/prepare/status",
...
def get_prepare_status():
    ...
    1. Query active task progress by task_id.
    2. Check whether preparation already exists by simulation_id.
    
    Request JSON:
        {
            "task_id": "task_xxxx",          // optional, task_id returned by prepare
            "simulation_id": "sim_xxxx"      // optional, simulation ID for prepared-state checks
        }
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "prepare_info": {...}            // Detailed information when ready
            }
        }
    """
    ...
    try:
        ...
        # Return an error if task_id is missing.
        if not task_id:
            if simulation_id:
                # simulation_id exists but preparation has not started.
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
        ...
        if not task:
            # The task is missing. If simulation_id is provided, check for an already-prepared simulation.
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Task is already complete because prepared data exists",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
...
def get_simulation(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to fetch simulation state: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverse the reports directory and find reports matching simulation_id.
    If multiple matches exist, return the newest one by created_at.
    ...
    # __file__ is app/api/simulation.py, so walk up two levels to backend/.
    ...
    try:
        ...
        # Sort by creation time descending and return the newest report.
...
def get_simulation_history():
    ...
    Used by the home page history view. Returns simulation records enriched with project metadata.
    ...
    try:
        ...
        # Enrich simulation data using only Simulation files.
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get real-time profile: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is already running, please call /stop first or use force=true to restart"
                            }), 400
...
def interview_agent():
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or closed. Please ensure the simulation has completed and is in command-waiting mode."
            }), 400
